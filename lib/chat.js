...
function chatHandler (socket) {
  ...
  socket.on('io:makeAMove',function(id){
    
    pub.hget('people', socket.client.conn.id, function (error, name) {
      // see: https://github.com/dwyl/hapi-error#handleerror-everywhere
      handleError(error, 'Error retrieving '
        + socket.client.conn.id + ' from Redis :-( for move: ' +  id);
    //console.log("pickboard received: " + size + " | from: " + name);
      mv = JSON.stringify({ // store each move as a JSON object
        s: id,
        t: new Date().getTime(),
      ...
      pub.rpush('game:moves', mv);   // game history
      pub.publish('game:moves:latest', mv);  // latest move
    });
...
function init (listener, callback) {
  ...
  pub.on('ready', function () {
    ...
    sub.on('ready', function () {
      sub.subscribe('chat:messages:latest', 'chat:people:new', 'game:boards:latest', 'game:moves:latest');
      ...
      sub.on('makeAMove', function (channel, makeAMove) {
         console.log(channel + ' : ' + makeAMove);
         io.emit(channel, makeAMove); // relay to all connected socket.io clients
       });
