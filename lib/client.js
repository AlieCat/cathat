...
$(document).ready(function () {
  ...
  //make a move?
  socket.on('game:moves:latest', function(data){
    var ndata=JSON.parse(data);
    var move = ndata.s;
    player = ndata.n;
    console.log('socket working '+ndata.n);
    renderMove(move, player);
  });
  
  function renderMove(move, player){
          //TODO change this to something other than alie
    if (player === 'alie'){
    $('#'+move).css("background-color","yellow");
    }else{
  ...
  function loadGame () {
    ...
    $.get('/loadGame', function (size) {
        ...
        createboard(board);
        //TODO load all the moves not just one
//         var move = NGobj.s;
//         var player = NGobj.n;
//         console.log('saved move '+move);        
//         size.forEach(function (move, player){
//           renderMove(move, player);
//           });
         });
